...
{
    m_filter1DelayLine.resize(numberFilter1Coefs - 1);
...
void HilbertShifter::processBlock(float *inputBlock, int numSamples)
{
    for (int i=0; i<numSamples; ++i)
    {
        ...
        for (int b=0; b<numberFilter1Coefs-1; ++b)
        {
            I += m_hilbert_B1_FilterCoefs[b+1] * m_filter1DelayLine[m_filter1DelayLine.size() - b];
        }
        
        
        float Q = windowedSample * m_hilbert_B2_FilterCoefs[0];
        for (int b=0; b<numberFilter2Coefs-1; ++b)
        {
            Q += m_hilbert_B2_FilterCoefs[b+1] * m_filter2DelayLine[m_filter2DelayLine.size() - b];
        }
        
        float u = fmodf(m_frequencyShift * i * (1/m_sampleRate), 1.f);
        float theta = 2 * juce::float_Pi * u;

        inputBlock[i] = I * std::cos(theta) + Q * sin(theta);
